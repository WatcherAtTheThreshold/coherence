...
2. **Every 3 turns**, each existing `~` spawns **1 adjacent** `~` (random cardinal direction)
3. New distortions also spread on subsequent cycles
4. Growth pattern (starting with 4 distortions): 4 → ~8 → ~16 → grid fills
5. Gives roughly **6-9 turns** below threshold before space becomes critical
6. Creates urgency: "Get what you need and get out"

**Spreading Implementation:**
- On trigger turns (3, 6, 9, etc.): snapshot all current `~` positions
- For each `~`: attempt to spawn adjacent in random cardinal direction
- If target tile is occupied or blocked, skip (no spawn)
- This prevents instant explosion while creating rising pressure
...
3. Is 6-9 turns below threshold the right window? (Too short = punishing, too long = no urgency)
4. Do artifacts feel like identity shifts, not just buffs?
5. Is knowing when to leave emotionally satisfying?
...
2. **Distortion spreading system**
   ...
   - Every 3 turns: spreading event
   - **Algorithm:**
     - Snapshot all current `~` positions
     - For each `~`: pick random cardinal direction
     - If that adjacent tile is `.` (ground), spawn new `~`
     - If occupied/blocked, skip (don't retry)
   - Expected growth: 4 → 8 → 16 distortions over 9 turns
   - Feel: rising water, not explosion
